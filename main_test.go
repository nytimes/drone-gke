...
// RunWaitDeployments is a helper function for testing waitForRollout.  For each flag-value 
// in flagValues it will expect a corresponding call of the form:  
// "kubectl rollout status <expected-value> ..."
func RunWaitForRollout(t *testing.T, flagName string, flagValues []string, expectedValues []string) {
	set := flag.NewFlagSet("test-set", 0)
	set.Int("wait-seconds", 256, "")
	set.String("namespace", "test-ns", "")
	strSlice := cli.StringSlice{}
	for _, s := range flagValues {
		strSlice.Set(s)
	}
	strSliceFlag := cli.StringSliceFlag{Name: flagName, Value: &strSlice}
...
func TestWaitForRollout(t *testing.T) {
	RunWaitForRollout(t, "wait-rollouts", []string{"deployment/d1"}, []string{"deployment/d1"})
	RunWaitForRollout(t, "wait-rollouts",
		[]string{"deployment/d1", "statefulset/s1"},
		[]string{"deployment/d1", "statefulset/s1"})
	RunWaitForRollout(t, "wait-deployments", []string{"d1"}, []string{"deployment/d1"})
	RunWaitForRollout(t, "wait-deployments",
		[]string{"d1", "d2"},
